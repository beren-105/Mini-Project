...
function SideBar(props) {
    ...
    return(
        ...
        <section className={`fixed top-0 left-0 bg-white w-56 h-full z-30 ${props.clickSideMenu ? null : '-translate-x-56'} xl:translate-x-0 dark:bg-black dark:text-white duration-300`}>
            {/* 로고 */}
            <div className="flex items-center ml-4 xl:disabled:opacity-75">
                <button
                    className=""
            ...
            <div className="px-4 text-sm h-full overflow-y-auto">
                <ul className="py-2 border-b dark:border-b-zinc-600">
                    {sideNav1.map(sideNav => (
                        <li key={sideNav.id}><a key={sideNav.id} className="flex items-center hover:bg-zinc-200 rounded-xl">
                            {sideNav.icon}
                            <span key={sideNav.id}>{sideNav.name}</span>
                        </a></li>
                    ))}
                </ul>
                <ul className="py-2 border-b dark:border-b-zinc-600">
